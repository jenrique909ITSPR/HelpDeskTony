...
/**
  ...
  * @var \App\Model\Entity\Ticketurgency $ticketurgency
  */
?>

<div class="ticketurgencies view">
    <h3><?= h($ticketurgency->name) ?></h3>
	<div class="actions">
		<ul>
			<li><?= $this->Form->postLink(__('Delete Ticketurgency'), ['action' => 'delete', $ticketurgency->id], ['confirm' => __('Are you sure you want to delete # {0}?', $ticketurgency->id)]) ?> </li>
			<li><?= $this->Html->link(__('New Ticketurgency'), ['action' => 'add']) ?> </li>
			<li><?= $this->Html->link(__('List Ticketurgencies'), ['action' => 'index']) ?> </li>
			<li><?= $this->Html->link(__('Edit Ticketurgency'), ['action' => 'edit', $ticketurgency->id]) ?> </li>
		</ul>
	</div>
	<div class="viewdata">
...
<div class="easyui-tabs">
    <div class="related" title="<?= __('Tickets') ?>">
        ...
        <?php if (!empty($ticketurgency->tickets)): ?>
        <table cellpadding="0" cellspacing="0">
		  ...
		  <tbody>
            <?php foreach ($ticketurgency->tickets as $tickets): ?>
            <tr>
                <td><?= h($tickets->id) ?></td>
